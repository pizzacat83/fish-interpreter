...
class FishProgram {
	...
	/**
	 * Width of each row of the program
	 *
	 * @type Integer[]
	 *
	 * @readonly
	 */
	get rowWidth() {
		return this._grid.map(line => line.length);
	}
	...
	advance() {
		...
		try {
			...
			// use space (Noops) for cells out of range
			const c = val === undefined? ' ' : String.fromCharCode(val);
			...
			this._hasTerminated = true;
			// Be very very kind and throw the actual error
			this._error = err;
		}
	}
